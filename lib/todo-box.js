...
function addCardToList(cardObj, position) {
  // allow for existing cards to have no tag attribute
  if (cardObj.tags === undefined) {
    cardObj.tags = [];
  }

  let complete = cardObj.complete ? "complete" : '';

  var tagsHTMLString = addTagsToCard(cardObj.tags);
  var importanceString = importanceIndexToString(cardObj.importance);
  var newCard =
    $(`<article class="card ${complete}" id="card-${cardObj.id}">
      <h2 class="card-title" contentEditable="true">${cardObj.title}</h2>
      <input class="card-button delete" type="button" name="name" value="">
      <p class="card-task" contentEditable="true">${cardObj.task}</p>
      <section class="card-tag-section"> <ul class="layout-flex-wrap card-tags">${tagsHTMLString}</ul></section>
      <input class="card-button upvote" type="button" name="name" value="">
      <input class="card-button downvote" type="button" name="name" value="">
      <section class="card-importance">Importance: <span class="importance-value">${importanceString}</span></section>
      <section> <button class='complete-button' type='button'>complete</button> </section>
    </article>`).hide().fadeIn('normal');
  updateVoteButtonStatus(cardObj.importance, newCard);
  ...
  for (let i = 0; i < localStorage.length; i += 1) {
    ...
    if (key.substring(0, 5) === "card-") {
      ...
      if (tagMatch || taskMatch !== -1 || titleMatch !== -1 || importanceMatch !== -1) {
        addCardToList(savedCardObject, 'prepend');
      }
    }
  }
}
...
function getSavedCards(getDone = false) {
  ...
  let selection = sortedCards.slice(0, 10);
...
function sortCards(sortDirection) {
  ...
  for (let i = 0; i < cards.length; i += 1) {
    addCardToList(sortedCards[i], 'prepend');
  }
}
