...
const inputFields = $('.title-field, .task-field, .tag-field');
...
saveButton.on('click', function () {
  let tagString = tagField.val();
  let title = titleField.val();
  let task = taskField.val();
  let tags = processTags(tagString);

  addTagsToTagBar(tags);
  saveTags(tags);

  count += 1;
  let cardObj = new Card(count, title, task, tags);
  writeCard(cardObj);
...
function addCardToList(options) {
  ...
  let tagsHTMLString = addTagsToCard(options.cardObj.tags);
...
function saveTags(tags) {
  let savedTagsString = localStorage.getItem('tags');
  let savedTagsArray = [];
...
function getMatchedCards(searchText) {
  for (let i = 0; i < localStorage.length; i += 1) {
    ...
    if (key.substring(0, 5) === "card-") {
      ...
      let tagMatch = cardObj.tags.includes(searchText);
